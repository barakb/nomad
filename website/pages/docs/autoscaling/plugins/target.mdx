...
in a number of ways including virtual machine identities. It is recommended, if
possible to use the [Vault Azure Secrets engine][vault_azure_backend] for supplying
access credentials to the plugin. Credentials should be injected into the configuration
via a template rather than as environment variables. This ensures the credentials
are passed only to the plugin, rather than being available for all plugins and
the agent process.
...
[azure_vmss_target]: /docs/autoscaling/plugins/target#azure-virtual-machine-scale-sets-target
